#include <string>

#include <string>

class Solution {
public:
    int myAtoi(std::string s) {
        bool isPositive = true;
        double result = 0;
        int strSize = s.size();
        if (s.size() == 0) return 0;
        // remove white space
        for (int i = 0; i < strSize; i++)
        {
            if (s.at(i) != ' '){
                s = s.substr(i, strSize - i);
                break;
            }
            if (i == strSize - 1) return 0;
        }
        if (s.at(0) == '-'){
            isPositive = false;
            s = s.substr(1, s.size() - 1);
        }
        else if (s.at(0) == '+')
        {
            s = s.substr(1, s.size() - 1);
        }
        for (auto &&c : s)
        {
            int val = convertDigit(c);
            if (val == -1) return result;
            result = result * 10 + val * (isPositive ? 1 : -1);
            if (result >= INT32_MAX){
                return INT32_MAX;
            }
            else if (result <= INT32_MIN){
                return INT32_MIN;
            }
        }
        return result;
    }
private:
    int convertDigit(char c){
        int res = c - '0';
        if (res > 9 || res < 0) return -1;
        return res;
    }
};

class Solution2 {
public:
    int myAtoi(std::string s) {
        double result = 0;
        int strSize = s.size(), index = 0, isPositive;
        // remove white space
        while (index < strSize && s[index] == ' '){
            index++;
        }
        if (index >= strSize) return 0;
        // determine sign
        if (s[index] == '-'){
            index++;
            isPositive = -1;
        }
        else if (s[index] == '+')
        {
            index++;
            isPositive = 1;
        }
        while (index < strSize && isdigit(s[index]))
        {
            int val = s[index] - '0';
            result = result * 10 + val * isPositive;
            if (result >= INT32_MAX){
                return INT32_MAX;
            }
            else if (result <= INT32_MIN){
                return INT32_MIN;
            }
            index++;
        }
        return result;
    }
};

enum State { q0, q1, q2, qd };

class StateMachine {
    // Store current state value.
    State currentState;
    // Store result formed and it's sign.
    int result, sign;

    // Transition to state q1.
    void toStateQ1(char& ch) {
        sign = (ch == '-') ? -1 : 1;
        currentState = q1;
    }

    // Transition to state q2.
    void toStateQ2(int digit) {
        currentState = q2;
        appendDigit(digit);
    }

    // Transition to dead state qd.
    void toStateQd() {
        currentState = qd;
    }

    // Append digit to result, if out of range return clamped value.
    void appendDigit(int& digit) {
        if ((result > INT32_MAX / 10) ||
            (result == INT32_MAX / 10 && digit > INT32_MAX % 10)) {
            if (sign == 1) {
                // If sign is 1, clamp result to INT_MAX.
                result = INT32_MAX;
            } else {
                // If sign is -1, clamp result to INT_MIN.
                result = INT32_MIN;
                sign = 1;
            }

            // When the 32-bit int range is exceeded, a dead state is reached.
            toStateQd();
        } else {
            // Append current digit to the result.
            result = result * 10 + digit;
        }
    }

public:
    StateMachine() {
        currentState = q0;
        result = 0;
        sign = 1;
    }

    // Change state based on current input character.
    void transition(char& ch) {
        if (currentState == q0) {
            // Beginning state of the string (or some whitespaces are skipped).
            if (ch == ' ') {
                // Current character is a whitespaces.
                // We stay in same state.
                return;
            } else if (ch == '-' || ch == '+') {
                // Current character is a sign.
                toStateQ1(ch);
            } else if (isdigit(ch)) {
                // Current character is a digit.
                toStateQ2(ch - '0');
            } else {
                // Current character is not a space/sign/digit.
                // Reached a dead state.
                toStateQd();
            }
        } else if (currentState == q1 || currentState == q2) {
            // Previous character was a sign or digit.
            if (isdigit(ch)) {
                // Current character is a digit.
                toStateQ2(ch - '0');
            } else {
                // Current character is not a digit.
                // Reached a dead state.
                toStateQd();
            }
        }
    }

    // Return the final result formed with it's sign.
    int getInteger() {
        return sign * result;
    }

    // Get current state.
    State getState() {
        return currentState;
    }
};

class LeetSolution {
public:
    int myAtoi(std::string s) {
        StateMachine Q;

        for (int i = 0; i < s.size() && Q.getState() != qd; ++i) {
            Q.transition(s[i]);
        }

        return Q.getInteger();
    }
};

#include <iostream>
int main(int argc, char const *argv[])
{
    Solution s;
    std::cout << s.myAtoi("-2147483647") << std::endl;
    return 0;
}
