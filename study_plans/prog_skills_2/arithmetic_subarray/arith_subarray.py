...
class Solution:  # noqa: D101
    def checkArithmeticSubarrays(  # spec
        self, nums: list[int], left_index: list[int], right_index: list[int]
    ) -> list[bool]:
        ...
        for left_ind, right_ind in zip(left_index, right_index, strict=False):
            ...
            ret_list.append(next(groups) and next(groups, False) is False)
        return ret_list
...
if __name__ == "__main__":
    ...
    sol.checkArithmeticSubarrays(
        [4, 6, 5, 9, 3, 7], left_index=[0, 0, 2], right_index=[2, 3, 5]
    )  # [True, False, True]
    sol.checkArithmeticSubarrays(
